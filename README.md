...
# Некоторые вещи мне  приходилось реализовывать самостоятельно в ручном режиме многократно.
# Что бы не делать больше одного и того же руками были написаны несколько плейбуков для ansible. 
...
# при изменении места потребуется соответствующим образом поменять имя файла mnt-storage.mount и его содержимое.
...
# - С начало он создаёт папки для директорий репозиториев и виртуальных машин.
# - Копирует файлы *.mount в системную папку.
...
#
# {{ dest_mount_iso }} - место, расположение папок с будущими репозитотриями
#
...
#
#-------playbook3.yml------
# Удостоверяемся, что следующие действия будут под пользователем root.
